...
class MatchingModel(nn.Module):
	...
	def forward(self, input):
		...
		feats = self.extractor(torch.unsqueeze(input, dim=1))
		# print(feats.shape)
		# print(feats.dtype)
		# print(pheno_a.dtype)
		########## deepgs change ##########
		...
		# y = self.estimator(feats)
		return torch.squeeze(feats,dim=1)
...
def train_epoch(model,device,dataloader,loss_fn,optimizer):
	...
	#print('starting to calculate training pearson')
	pearson = np.corrcoef(y_pred,y_true)[0,1]
	#print('calculated training pearson')
...
def valid_epoch(model,device,dataloader,loss_fn):
	...
	with torch.no_grad():
		...
		for feat, label in dataloader:
			...
			y_true.extend(label.tolist())
			y_pred.extend(output.tolist())
	# print('starting to calculate validation pearson')
	pearson = np.corrcoef(y_pred,y_true)[0,1]
	# print('y_pred:',y_pred)
	# print('y_true:',y_true)
	# print('calculated validation pearson')
...
def predict_all(marker_test_matrix, model, device):
	predictions = []
	model.eval()
	with torch.no_grad():
		# for geno_a in marker_test_matrix:
		# 	predictions.append(model(torch.unsqueeze(geno_a,0)).item())
		predictions = model(marker_test_matrix)
	return predictions
...
if __name__ == '__main__':
	...
	experiment_props = {'Model':'deepgs', 'Trait': args.trait, 'epochs':num_epochs, 'batch_size':batch_size, 'folds':k}
	...
	with open(f'results/feature_sel/{args.dataset}/{args.trait}/deepgs/fold_split_info.txt','w') as f:
		pass 

	with open(f'results/feature_sel/{args.dataset}/{args.trait}/deepgs/seeds/deepgs_seed_{args.seed}.csv','w') as f:
	...
	for k_feat in k_feats:
		...
		for fold, (train_idx,val_idx) in enumerate(splits.split(np.arange(X.shape[0]))):
			...
			# with open(f'results/feature_sel/{args.dataset}/{args.trait}/deepgs/fold_split_info.txt','a') as f:
			...
			experiment_props.update({'optimizer':'SGD','momentum':momentum,'weight_decay':weight_decay,'loss':'MSE'})
			...
			for epoch in range(num_epochs):
				...
				if epoch % num_epochs == num_epochs-1:
					y_true = y_test
					y_pred = np.array(predict_all(marker_test_matrix, model, device).cpu())
		...
		# plt.savefig(f'results/feature_sel/{args.dataset}/{args.trait}/deepgs/cols_{X_train.shape[1]}.pdf')
		# plt.close()
		with open(f'results/feature_sel/{args.dataset}/{args.trait}/deepgs/seeds/deepgs_seed_{args.seed}.csv','a') as f:
	...
	# plt.savefig(f'results/feature_sel/{args.dataset}/{args.trait}/deepgs/inflection_plot.pdf')