...
from sklearn.linear_model import LinearRegression, Lasso, ElasticNet, Ridge, LinearRegression
...
if __name__ == '__main__':
	...
	parser.add_argument('--linmodel',type=str)
	...
	experiment_props = {'Model':args.linmodel, 'Trait': args.trait, 'folds':k}
	...
	with open(f'results/feature_sel/{args.dataset}/{args.trait}/{args.linmodel}/fold_split_info.txt','w') as f:
		pass 

	with open(f'results/feature_sel/{args.dataset}/{args.trait}/{args.linmodel}/{args.linmodel}.csv','w') as f:
	...
	for k_feat in k_feats:
		...
		for fold, (train_idx,val_idx) in enumerate(splits.split(np.arange(X.shape[0]))):
			...
			with open(f'results/feature_sel/{args.dataset}/{args.trait}/{args.linmodel}/fold_split_info.txt','a') as f:
			...
			if args.linmodel == 'lasso':
				model = Lasso()
			elif args.linmodel == 'ridge':
				model = Ridge()
			elif args.linmodel == 'elasticnet':
				model = ElasticNet()
			elif args.linmodel == 'linregr':
		...
		plt.savefig(f'results/feature_sel/{args.dataset}/{args.trait}/{args.linmodel}/cols_{X_train.shape[1]}.pdf')
		plt.close()
		with open(f'results/feature_sel/{args.dataset}/{args.trait}/{args.linmodel}/{args.linmodel}.csv','a') as f:
	...
	plt.savefig(f'results/feature_sel/{args.dataset}/{args.trait}/{args.linmodel}/inflection_plot.pdf')