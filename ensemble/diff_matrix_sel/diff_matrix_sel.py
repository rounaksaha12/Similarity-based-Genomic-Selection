...
def difference_matrix(model,X,y):
	n = X.shape[0]
	dif = np.zeros((n,n))
	model.eval()
	with torch.no_grad():
		for i in range(n):
			for j in range(n):
				predicted = model(torch.unsqueeze(X[i],0),torch.unsqueeze(X[j],0),torch.unsqueeze(y[i],0))
				true = y[j]
				dif[i][j] = abs(predicted.item()-true)

	return dif

def choose_best_subset_mask(model,X,y,k_best,epsilon=1e-6):
	dif = difference_matrix(model,X,y)
	aggr_err = [np.sum(dif[i]) for i in range(len(dif))]
	aggr_err = np.array(aggr_err)
	# get the indices of k_best elements with lowest aggr_err
	indices = np.argsort(aggr_err)[:k_best]
...
if __name__ == '__main__':
	...
	with open(f'results/diff_matrix_sel/{args.dataset}/{args.trait}/fold_split_info.txt','w') as f:
		pass

	for fold, (train_idx,val_idx) in enumerate(folds):

		with open(f'results/diff_matrix_sel/{args.dataset}/{args.trait}/fold_split_info.txt','a') as f:
		...
		dif = difference_matrix(model,marker_train_matrix,traits_train)
		fig, axis = plt.subplots(figsize=(0.1*marker_train_matrix.shape[0],0.1*marker_train_matrix.shape[0]))
		cmap = mpl.colormaps['YlOrBr']
		im = axis.imshow(dif,cmap=cmap,vmin=np.min(dif),vmax=np.max(dif))
		axis.set_xticks(np.arange(marker_train_matrix.shape[0]))
		axis.set_yticks(np.arange(marker_train_matrix.shape[0]))
		axis.set_xticklabels(np.arange(marker_train_matrix.shape[0]))
		axis.set_yticklabels(np.arange(marker_train_matrix.shape[0]))
		fig.colorbar(im,ax=axis)
		axis.set_title(f'Fold {fold+1} difference matrix')
		fig.savefig(f'results/diff_matrix_sel/{args.dataset}/{args.trait}/fold_{fold+1}_diff_matrix.pdf')
		plt.close(fig)

		aggr_err = [np.sum(dif[i]) for i in range(dif.shape[0])]
		aggr_err = np.array(aggr_err)
		indices = np.argsort(aggr_err)
		...
		for frac_idx, frac in enumerate(fracs):
			subset_mask = indices[:int(frac*len(indices))]
			y_true = np.array(traits_test.cpu())
			y_pred = np.array(predict_all(marker_test_matrix, marker_train_matrix[subset_mask], traits_train[subset_mask], model, device))
			# performance of frac with model trained on fold
			pearson_r = np.corrcoef(y_true, y_pred)[0,1]
			mse = np.mean((y_true-y_pred)**2)
			print(f'Fold {fold+1} fraction={frac} pearson_r={pearson_r} mse={mse}')
			plot_scatter(scatter_axes[frac_idx], fold, y_true, y_pred,pearson_r,mse)
			temp1.append(pearson_r)
			temp2.append(mse)
	...
	for i in range(len(fracs)):
		experiment_props['frac']=fracs[i]
		experiment_props['Avg_PCC']=pearson_fracs[i]
		experiment_props['Avg_MSE']=mse_fracs[i]
		scatter_figs[i].suptitle(str(experiment_props),wrap=True)
		scatter_figs[i].savefig(f'results/diff_matrix_sel/{args.dataset}/{args.trait}/frac_{fracs[i]}.pdf')
		plt.close(scatter_figs[i])

	with open(f'results/diff_matrix_sel/{args.dataset}/{args.trait}/summary.csv','w') as f: # to erase out the previous contents of the file
		pass

	with open(f'results/diff_matrix_sel/{args.dataset}/{args.trait}/summary.csv','a') as f:
	...
	plt.savefig(f'results/diff_matrix_sel/{args.dataset}/{args.trait}/Comparison_across_different_samples.pdf')
