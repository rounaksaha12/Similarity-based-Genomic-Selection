...
def reference_matrix(model,reference,input,y_reference): # X_train: reference, X_test: input, y_train: y_reference
	m = reference.shape[0]
	n = input.shape[0]
	ref = np.zeros((m,n))
	model.eval()
	with torch.no_grad():
		for i in range(m):
			for j in range(n):
				predicted = model(torch.unsqueeze(reference[i],0),torch.unsqueeze(input[j],0),torch.unsqueeze(y_reference[i],0))
...
if __name__ == '__main__':
	...
	with open(f'training_cache/{args.dataset}/{args.trait}/fold_split_info.txt','w') as f:
		pass

	for fold, (train_idx,val_idx) in enumerate(folds):

		with open(f'training_cache/{args.dataset}/{args.trait}/fold_split_info.txt','a') as f:
		...
		train_train = reference_matrix(model,marker_train_matrix,marker_train_matrix,traits_train) # i,j-th entry is the predicted trait of j-th test sample using i-th train sample as reference
		train_test = reference_matrix(model,marker_train_matrix,marker_test_matrix, traits_train) # i,j-th entry is predicted trait of j-th test sample using i-th train sample as reference

		######## plot the train_test matrix ########
		fig, axis = plt.subplots(figsize=(0.1*marker_test_matrix.shape[0],0.1*marker_train_matrix.shape[0]))
		cmap = mpl.colormaps['YlOrBr']
		im = axis.imshow(np.abs(train_test-y_test),cmap=cmap,vmin=np.min(np.abs(train_test-y_test)),vmax=np.max(np.abs(train_test-y_test)))
		...
		axis.set_title(f'Fold {fold+1} train-test reference matrix')
		fig.savefig(f'training_cache/{args.dataset}/{args.trait}/fold_{fold+1}_train-test_ref_matrix.pdf')
		plt.close(fig)

		######## plot the train_train matrix ########
		fig, axis = plt.subplots(figsize=(0.1*marker_train_matrix.shape[0],0.1*marker_train_matrix.shape[0]))
		cmap = mpl.colormaps['YlOrBr']
		im = axis.imshow(np.abs(train_train-y_train),cmap=cmap,vmin=np.min(np.abs(train_train-y_train)),vmax=np.max(np.abs(train_train-y_train)))
		axis.set_xticks(np.arange(marker_train_matrix.shape[0]))
		axis.set_yticks(np.arange(marker_train_matrix.shape[0]))
		axis.set_xticklabels(np.arange(marker_train_matrix.shape[0]))
		axis.set_yticklabels(np.arange(marker_train_matrix.shape[0]))
		fig.colorbar(im,ax=axis)
		axis.set_title(f'Fold {fold+1} train-train reference matrix')
		fig.savefig(f'training_cache/{args.dataset}/{args.trait}/fold_{fold+1}_train-train_ref_matrix.pdf')
		plt.close(fig)


		# save the model, x_train, x_test, y_train, y_test, train_train, train_test
		model_filename = f'training_cache/{args.dataset}/{args.trait}/fold_{fold+1}_model.bin'
		...
		dict_to_save.append(fold_dict)

	# dump dict_to_save into a pickle file
	with open(f'training_cache/{args.dataset}/{args.trait}/fold_info.pickle','wb') as f:
